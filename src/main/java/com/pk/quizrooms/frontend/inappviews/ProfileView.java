...
public class ProfileView extends VerticalLayout {
    ...
    private void showCreatedQuizzes() {
        ...
        grid.addComponentColumn(quiz -> {
          Button view = new Button("View who played your quiz");
          view.setThemeVariants(ButtonVariant.LUMO_PRIMARY);
        ...
        close.setThemeVariants(ButtonVariant.LUMO_ERROR);
    ...
    private void showPlayedQuizzes() {
        Dialog dialog = new Dialog("PlayedByYou");
        dialog.setSizeFull();

        Grid<QuizPlayedBy> grid = new Grid<>(QuizPlayedBy.class, false);
        grid.addColumn(q -> q.getQuiz().getTitle()).setHeader("Quiz");
        grid.addColumn(q-> q.getQuiz().getQuizhashid()).setHeader("Quiz ID");
        grid.addColumn(q->q.getQuiz().getTotalmarks()).setHeader("Total Score");
        grid.addColumn(QuizPlayedBy::getScore).setHeader("Score Obtained");
        grid.addComponentColumn(quizplayedby -> {
            Button playagain = new Button("Playagain");
            playagain.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
            playagain.addClickListener(e->
            {
                Dialog passworddialog = new Dialog("Enter password to play");
                if (quizplayedby.getQuiz().getRequirepassword().toString().equalsIgnoreCase("yes")) {
                    dialog.open();
                    ...
                    ok.addClickListener(event -> {
                        ...
                        if (pass.getValue().equals(quizplayedby.getQuiz().getQuizpass())) {
                            passworddialog.close();
                            UI.getCurrent().navigate(PlayQuiz.class,quizplayedby.getQuiz().getQuizhashid());
                        } else
